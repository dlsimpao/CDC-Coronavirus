...
```
```{r}
Acf(cdc_xts_diff1, lag.max=60, plot=FALSE)
```
...
```{r}
#Acf gradually declines.
Acf(cdc_xts_may_diff1, lag.max = 60, plot = FALSE)

```

#PACF: Nov 2020 
```{r}
Pacf(cdc_xts_diff1, lag.max = 60)
```

```{r}
#Pacf cuts off at lag 7
Pacf(cdc_xts_diff1, lag.max=60, plot=FALSE)
```
# PACF: May 2021
```{r}
Pacf(cdc_xts_may_diff1, lag.max = 60)
```

```{r}
#Pacf cuts off at lag 7
Pacf(cdc_xts_may_diff1, lag.max=60, plot=FALSE)
```
