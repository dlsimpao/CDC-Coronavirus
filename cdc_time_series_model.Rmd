...
png(file = "allmay_sarima_5days.png", width = 1080, height = 610)
...
ggplot(data = allmay_full, aes(x = Date, y = Values, color = Group, group = 1)) + geom_line() + coord_x_date(xlim = c("2021-03-01","2021-15-05"), ylim = (c(-10,20000))) + ggtitle("Time Series 5 day forecast (adjusted for negative values)")

ggsave("allmay_ggplot_5days.png")
```
...
```{r}
days = 60
png(file = "allmay_sarima_60days.png", width = 1080, height = 610)
...
actual_toforecast_may = cdc_allmay.ts %>% 
  filter(Date > tail(apr_nov2020$Date) + 1) %>% 
  filter(Date < tail(apr_nov2020$Date) + days)
